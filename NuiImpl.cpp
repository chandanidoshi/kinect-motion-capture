...
std::vector<std::vector<float>> masterPositions;
void writePositions(std::vector<std::vector<float>> masterPositions);
...
void CSkeletalViewerApp::Nui_DrawSkeleton( NUI_SKELETON_DATA * pSkel, HWND hWnd, int WhichSkeletonColor )
{
    ...
    USHORT depth;
	//std::ofstream myFile;
	//myFile.open("C:/Users/Alexis/Desktop/evan/newFile.txt");
	//myFile<<"POSITION: "<<pSkel->Position.x<<" "<<pSkel->Position.y<<" "<<pSkel->Position.z<<"\n";
	std::vector<float> singleSkeletonPositions;
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; i++)
    {
		...
		//myFile << pSkel->SkeletonPositions[i].x <<","<<pSkel->SkeletonPositions[i].y<<","<<pSkel->SkeletonPositions[i].z<<"\n";
		singleSkeletonPositions.push_back(pSkel->SkeletonPositions[i].x);
		singleSkeletonPositions.push_back(pSkel->SkeletonPositions[i].y);
		singleSkeletonPositions.push_back(pSkel->SkeletonPositions[i].z);
    }
	masterPositions.push_back(singleSkeletonPositions);
	writePositions(masterPositions);
	//myFile<<"\n";
	//myFile.close();
...
void writePositions(std::vector<std::vector<float>> masterPositions){
	std::ofstream myFile;
	myFile.open("C:/Users/Alexis/Desktop/evan/jump.csv");
	for (int i=0; i<masterPositions.size(); i++){
		for (int j=0; j<masterPositions[i].size(); j++){
			myFile<<masterPositions[i][j]<<",";
		}
		myFile<<"\n";
	}
	myFile.close();
